...
bool bs_getQueueName(const thread_t thread, char* const buffer, int bufLength);
    
bool bs_getThreadName(const thread_t thread, char* const buffer, int bufLength);
...
bool bs_getQueueName(const thread_t thread, char* const buffer, int bufLength) {
    ...
    bufLength = __ISMin(length, bufLength - 1);
...
bool bs_backtraceOfAllThread(uintptr_t **allThreadBuffer,
    ...
    *threadCount = thread_count;
    for(int i = 0; i < __ISMin(thread_count, maxThreadCount); i++) {
        if (!bs_getQueueName(threads[i], threadsName[i], 50)) {
            bs_getThreadName(threads[i], threadsName[i], 50);
        }
